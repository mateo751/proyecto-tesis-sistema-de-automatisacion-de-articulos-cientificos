...
const API_URL = 'http://127.0.0.1:5000/preguntas';  // Asegúrate de que el puerto sea el correcto
...
export const crearPregunta = async (data) => {
  try {
    ...
    const preguntaResponse = responseData.pregunta_response || 'No se generó preguntas response';
    
    return {
      mensaje: responseData.mensaje,
      id: responseData.id,
      pregunta_response: preguntaResponse  // Siempre devolver un valor para 'pico_response'
    };
  } catch (error) {
    console.error('Error en crear preguntas:', error);
